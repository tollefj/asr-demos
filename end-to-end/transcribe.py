import shutil
import os
from pydub import AudioSegment
from pydub.playback import play
from rich import print
from argparse import ArgumentParser
from tqdm import tqdm
from transformers import pipeline
from util import make_timestamp

OUTPUT = "output-segments"

def split_audio(input_file, segment_length_ms=30000, overlap_ms=0, max_segments=None, delete=False):
    if os.path.exists(OUTPUT) and "info.txt" in os.listdir(OUTPUT) and not delete:
        with open(f"{OUTPUT}/info.txt", "r") as f:
            if f.read() == input_file:
                print("[green]Already split. Continuing...[/green]")
                return

    audio = AudioSegment.from_mp3(input_file)
    segment_number = 1
    start_time = 0

    shutil.rmtree(OUTPUT, ignore_errors=True)
    os.makedirs(OUTPUT, exist_ok=True)
    with open(f"{OUTPUT}/info.txt", "w") as f:
        f.write(input_file)

    if not max_segments:
        max_segments = 999999
    with tqdm(total=len(audio), desc="Splitting audio file") as pbar:
        while start_time < len(audio):
            segment = audio[start_time:start_time + segment_length_ms]
            
            output_file = f"{OUTPUT}/{segment_number}.mp3"
            segment.export(output_file, format="mp3")
            
            start_time += segment_length_ms - overlap_ms
            segment_number += 1
            pbar.update(segment_length_ms - overlap_ms)
            if segment_number > max_segments:
                break

if __name__ == "__main__":
    argparser = ArgumentParser()
    argparser.add_argument("-f", "--file", type=str, help="Input file (mp3, wav, etc.)", required=True)
    argparser.add_argument("-d", "--delete", action="store_true", help="Delete output directory")
    argparser.add_argument("-s", "--speed", type=str, help="Model speed (superfast (151MB)/fast (290MB)/normal (967MB)/slow(3.06GB)). Slow is more precise.", default="normal")
    argparser.add_argument("-a", "--audio", action="store_true", help="Play audio while transcribing")

    args = argparser.parse_args()
    inp = args.file
    print(f"[yellow]Splitting audio file {inp} into segments of 30 seconds[/yellow]")
    split_audio(inp, delete=args.delete)
    
    model_size = {
        "superfast": "tiny",
        "fast": "base",
        "normal": "small",
        "slow": "medium",
    }
    model_id = f"NbAiLab/nb-whisper-{model_size[args.speed]}-beta"
    print(f"[yellow]Loading model {model_id}[/yellow]")
    asr = pipeline("automatic-speech-recognition", model_id)
    print(f"[yellow]Transcribing segments...[/yellow]")

    valid_files = [f for f in os.listdir(OUTPUT) if ".txt" not in f]
    srt_filename = "output.srt"
    if os.path.exists(srt_filename):
        os.remove(srt_filename)
    
    print(f"[yellow]Writing output to {srt_filename}[/yellow]")
    
    counter = 1
    total_time = 0

    with open(srt_filename, "w", encoding="utf-8") as srt_file:
        for sample in sorted(valid_files, key=lambda x: int(x.split(".")[0])):
            fpath = os.path.join(OUTPUT, sample)
            audio = AudioSegment.from_file(fpath)
            kwargs = {"task": "transcribe", "language": "no"}
            res = asr(fpath, generate_kwargs=kwargs, return_timestamps=True)
            chunks = res["chunks"]
            timestamps = [c["timestamp"] for c in chunks]
            text = [c["text"] for c in chunks]

            for (start, end), txt in zip(timestamps, text):
                start_time = float(start) * 1000
                end_time = float(end) * 1000
                txt = txt.strip()
                start_srt = make_timestamp(total_time, start)
                end_srt = make_timestamp(total_time, end)
                srt_entry = f"{counter}\n{start_srt} --> {end_srt}\n{txt}\n"
                srt_file.write(srt_entry)
                if args.audio:
                    print(srt_entry)
                    play(audio[start_time:end_time])
                counter += 1
            total_time += 30
    
    
    